...
public class GridSizeMigrationTask {
    ...
    protected ArrayList<DbEntry> loadWorkspaceEntries(long screen) {
        ...
        final int titleId = c.getColumnIndexOrThrow(Favorites.TITLE);
        ...
        while (c.moveToNext()) {
            ...
            entry.title = c.getString(titleId);
            ...
            try {
                ...
                switch (entry.itemType) {
                    ...
                    case Favorites.ITEM_TYPE_FOLDER: {
                        int total = getFolderItemsCount(entry.id);
                        // 推荐文件特殊处理，不做空判断
                        if (total == 0 && !RecommendAppManager.isFeaturedFolder(entry.title.toString())) {
                            throw new Exception("Folder is empty");
                        }
                        entry.weight = WT_FOLDER_FACTOR * total;
                        break;
