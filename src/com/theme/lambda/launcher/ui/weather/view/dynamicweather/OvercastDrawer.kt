 ...
 * 阴天
 */
class OvercastDrawer constructor(context: Context, isNight: Boolean) :
    BaseDrawer(context, isNight) {
    //	final ArrayList<CloudHolder> holders = new ArrayList<CloudHolder>();
    private val paint: Paint = Paint(Paint.ANTI_ALIAS_FLAG)
    val holders: ArrayList<CloudyDrawer.CircleHolder> = ArrayList<CloudyDrawer.CircleHolder>()
    override fun setSize(width: Int, height: Int) {
        super.setSize(width, height)
        if (holders.size == 0) {
            holders.add(
                CloudyDrawer.CircleHolder(
                    0.20f * width,
                    -0.30f * width,
                    0.06f * width,
                    0.022f * width,
    ...
    override fun drawWeather(canvas: Canvas, alpha: Float): Boolean {
        for (holder: CloudyDrawer.CircleHolder in holders) {
    ...
    override fun getSkyBackgroundGradient(): IntArray {
        return if (isNight) SkyBackground.OVERCAST_N else SkyBackground.OVERCAST_D
    }
}