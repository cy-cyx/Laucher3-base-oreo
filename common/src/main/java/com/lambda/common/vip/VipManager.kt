...
object VipManager {
    ...
    fun init(debug: Boolean, secretKey: String) {
        if (init) return
        init = true
        Billing.isDebug = debug
        isVip.value = SpKey.isVip.getSpBool()
        // 延迟请求网络有坑，归因问题
        GlobalScope.launch {
            delay(3000)
            Billing.init(InitParam.Builder(Constants.BASE_URL, secretKey).build())
            upDataFreeAdUntil()
        }
    }
    ...
    fun upDataFreeAdUntil() {
        ...
        Billing.getFreeAdUntil(object : Callback<UserAssets?> {
